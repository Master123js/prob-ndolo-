...
function canInteract(mod, target, action, channel) {
  if (!memberInteract(mod, target)) {
    sendMessage(channel, `¡Ups! No puedes \`${action}\` ${target.user.tag}`);
    return false;
  }

  if (!target.manageable) {
    sendMessage(channel, `¡Ups! No puedo \`${action}\` ${target.user.tag}`);
    return false;
  }
...
async function setupMutedRole(guild) {
  ...
  try {
    ...
    console.log("Error de creación de rol silenciado: " + ex);
    return mutedRole;
  }
}
...
async function purgeMessages(message, type, amount, argument) {
  ...
  if (toDelete.size == 0) {
    return sendMessage(channel, "¡Mensajes no encontrados que se pueden purgar!");
  }
  ...
  try {
    ...
    console.log(ex);
    sendMessage(channel, "Purga fallida");
  }

  if (deletedMessages) {
    const sentMsg = await sendMessage(channel, `Purgado con éxito ${deletedMessages.size} mensajes`);
    sentMsg
      .delete({
        timeout: 3000,
      })
