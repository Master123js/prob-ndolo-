  ...
  client.on("messageReactionAdd", async (reaction, user) => {
    ...
    if (emoji.name?.length === 4) {
      ...
      getCountryLanguages(countryCode, async (err, languages) => {
        ...
        // filtrar idiomas para los que está disponible la traducción de Google
        ...
        // elimina el inglés si hay otros códigos de idioma
        ...
        let head = `Mensaje Original: [Aquí](${message.url})\nLenguaje Fuente: ${src}\n\n`;
        const embed = new MessageEmbed()
          .setColor(EMBED_COLORS.BOT_EMBED)
          .setAuthor(`Traducción`)
          .setDescription(head + desc)
          .setFooter(`Solicitado por ${user.tag}`, user.displayAvatarURL());
