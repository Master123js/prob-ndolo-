  ...
  client.on("messageCreate", async (message) => {
    ...
    if (cmd) {
      ...
      try {
        ...
        sendMessage(message.channel, "¡Ups! Ocurrió un error al ejecutar el comando");
        console.log(ex);
      }
    }
  });
...
function addCommand(cmd) {
  ...
  if (COMMAND_INDEX.has(cmd.name)) {
    throw new Error(`Comando ${cmd.name} ya registrado`);
  }

  cmd.aliases.forEach((alias) => {
    if (COMMAND_INDEX.has(alias)) {
      throw new Error(`Alias ${alias} ya registrado`);
    }
